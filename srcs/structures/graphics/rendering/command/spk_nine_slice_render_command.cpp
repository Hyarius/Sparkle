...
namespace
{
	...
	{
		return {static_cast<float>(p_x), static_cast<float>(p_y), p_depth};
	}

	void validateArguments(const spk::SpriteSheet& p_spriteSheet, const spk::Rect2D& p_screenRect, const spk::Vector2Int& p_cornerSize)
	{
...
namespace spk
{
	NineSliceRenderCommand::NineSliceRenderCommand(
		const spk::SpriteSheet& p_spriteSheet, spk::Rect2D p_screenRect, spk::Vector2Int p_cornerSize, float p_depth)
	{
		validateArguments(p_spriteSheet, p_screenRect, p_cornerSize);

		const std::array<int, 4> xPositions = {
			p_screenRect.left(), p_screenRect.left() + p_cornerSize.x, p_screenRect.right() - p_cornerSize.x, p_screenRect.right()};
		const std::array<int, 4> yPositions = {
			p_screenRect.top(), p_screenRect.top() + p_cornerSize.y, p_screenRect.bottom() - p_cornerSize.y, p_screenRect.bottom()};
		...
		_textureCommand = std::make_unique<spk::DrawTextureMeshRenderCommand>(static_cast<const spk::Texture&>(p_spriteSheet), mesh);
	}
