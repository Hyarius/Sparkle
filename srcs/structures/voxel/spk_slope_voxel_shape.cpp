...
namespace spk
{
	...
	void SlopeVoxelShape::_constructRenderFaces()
	{
		...
		faces.innerFaces.push_back(createFace(createPolygon("slope", slopePositions, slopeUVs)));
		...
		faces.outer(spk::VoxelAxisPlane::PositiveX) = createFace(createVerticalTriangle(
			"sideRight", {1, 0, 1}, {1, 0, 0}, {1, 1, 1}));
		faces.outer(spk::VoxelAxisPlane::NegativeX) = createFace(createVerticalTriangle(
