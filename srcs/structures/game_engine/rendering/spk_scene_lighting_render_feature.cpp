...
namespace
{
	...
	[[nodiscard]] bool validColor(const spk::Color &value)
	{
		return finiteNonNegative(value.r) && finiteNonNegative(value.g) && finiteNonNegative(value.b);
	}
		...
		GLuint binding, const std::vector<TRecord> &records)
	{
		...
		if (!records.empty())
		{
			buffer->edit(records.data(), records.size() * sizeof(TRecord));
		}
		return buffer;
	}
}
...
namespace spk
{
	...
	const spk::EnvironmentLighting &SceneLightingRenderFeature::environmentLighting() const noexcept
	{
		return _environment;
	}
	const spk::SceneLightingDiagnostics &SceneLightingRenderFeature::diagnostics() const noexcept
	{
		return _diagnostics;
	}
	...
	const spk::DirectionalShadowSettings &SceneLightingRenderFeature::directionalShadowSettings() const noexcept
	{
	...
	void SceneLightingRenderFeature::prepareFrame(const spk::SceneRenderBuildContext &context)
	{
		...
		for (spk::Component *component : context.components.components<spk::Light3D>())
		{
			...
			{
				++_diagnostics.processableDirectional;
				directional.push_back(candidate);
			}
			else if (light->isPoint() && finiteNonNegative(light->point().range) && light->point().range > 0.0f)
			{
				++_diagnostics.processablePoint;
				point.push_back(candidate);
			}
			else if (light->isSpot() && finiteNonNegative(light->spot().range) && light->spot().range > 0.0f)
			{
				++_diagnostics.processableSpot;
				spot.push_back(candidate);
			}
		}
		...
		{
			if (candidate.light->castsShadows())
			{
				shadowOwner = candidate.light;
				break;
			}
		}
		...
		if (shadowOwner != nullptr && std::none_of(directional.begin(), directional.end(), [shadowOwner](const Candidate &candidate) {
				return candidate.light == shadowOwner;
			}))
		{
			Candidate replacement{.light = shadowOwner};
			if (context.mainCamera != nullptr)
			{
				replacement.distanceSquared = shadowOwner->transform().worldPosition().squaredDistance(context.mainCamera->position());
			}
			directional.back() = replacement;
			std::sort(directional.begin(), directional.end(), ordered);
		}
		point.resize(std::min(point.size(), MaxPointLights));
		spot.resize(std::min(spot.size(), MaxSpotLights));
		_diagnostics.selectedDirectional = directional.size();
		_diagnostics.selectedPoint = point.size();
		_diagnostics.selectedSpot = spot.size();
		...
		if (shadowOwner != nullptr && context.mainCamera != nullptr && context.mainCamera->nearPlane() > 0.0f && context.mainCamera->farPlane() > context.mainCamera->nearPlane())
		{
			...
			{
				if (directional[index].light == shadowOwner)
				{
					header.shadowDirectionalIndex = static_cast<std::int32_t>(index);
				}
			}
			if (header.shadowDirectionalIndex >= 0)
			{
				...
				if (farDistance > nearDistance)
				{
					...
					{
						_shadowTargets.clear();
						_shadowTargets.reserve(_shadowSettings.cascadeCount);
						for (std::size_t i = 0; i < _shadowSettings.cascadeCount; ++i)
						{
							_shadowTargets.push_back(std::make_shared<spk::FrameBufferObject>(spk::FrameBufferObject::depthTextureTarget(_shadowSettings.resolution, spk::Texture::Format::Depth24)));
						}
					}
					...
					for (std::size_t i = 0; i < _shadowSettings.cascadeCount; ++i)
					{
						...
						for (const auto &corner : corners)
						{
							radius = std::max(radius, corner.distance(center));
						}
						...
						if (i == 0)
						{
							resources->shadow.cascadeFarDistances.x = split;
						}
						else if (i == 1)
						{
							resources->shadow.cascadeFarDistances.y = split;
						}
						else if (i == 2)
						{
							resources->shadow.cascadeFarDistances.z = split;
						}
						else
						{
							resources->shadow.cascadeFarDistances.w = split;
						}
						previous = split;
					}
					...
					resources->shadowTargets = _shadowTargets;
					resources->cascadeCount = _shadowSettings.cascadeCount;
				}
			}
		}
		headerBuffer->edit(&header, sizeof(header));
		...
		shadowBuffer->edit(&resources->shadow, sizeof(resources->shadow));
	...
	void SceneLightingRenderFeature::contribute(const spk::SceneRenderBuildContext &context)
	{
		...
		for (const auto type : {spk::SceneRenderPasses::MainOpaque, spk::SceneRenderPasses::MainTransparent, spk::SceneRenderPasses::MainOverlay})
		{
			...
			for (std::size_t index = 0; index < _frame->shadowTargets.size() && index < targets.size(); ++index)
			{
				targets[index] = _frame->shadowTargets[index];
			}
	...
	{
		if (_frame == nullptr || _frame->cascadeCount == 0)
		{
			return;
		}
