...
namespace spk
{
	...
	std::unique_ptr<spk::RenderContext> PlatformRuntime::createRenderContext(spk::IFrame &p_frame)
	{
		spk::Frame &frame = _requireFrame<spk::Frame>(p_frame);
		return std::make_unique<RenderContext>(frame);
	}

	void PlatformRuntime::waitUntilWorkDone()
	{
		glFinish();
	}

	void PlatformRuntime::saveScreenshot(const std::filesystem::path &p_outputPath, const spk::Rect2D &p_rect) const
	{
		const int width = static_cast<int>(p_rect.width());
		const int height = static_cast<int>(p_rect.height());
		if (width <= 0 || height <= 0)
		{
			throw std::runtime_error("spk::PlatformRuntime::saveScreenshot requires a non-empty capture rect");
		}
		...
		{
			throw std::runtime_error("spk::PlatformRuntime::saveScreenshot failed to write [" + p_outputPath.string() + "]");
		}
	}

	void PlatformRuntime::saveScreenshot(const std::filesystem::path &p_outputPath) const
	{
