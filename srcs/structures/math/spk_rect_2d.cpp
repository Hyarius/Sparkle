...
namespace spk
{
	...
	{
		return p_other.left() >= left() && p_other.top() >= top() && p_other.right() <= right() && p_other.bottom() <= bottom();
	}
	...
	Rect2D Rect2D::shrink(const spk::Vector2Int& p_offset) const
	{
		...
		return Rect2D(newLeft, newTop, static_cast<std::size_t>(newRight - newLeft), static_cast<std::size_t>(newBottom - newTop));
	}
