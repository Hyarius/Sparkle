...
namespace pg
{
	...
	TownWorkspace::TownWorkspace(const WorldPlan &p_plan, PlanTownSite p_site, TownComposition p_composition) :
		_plan(p_plan),
		_site(std::move(p_site)),
		_composition(std::move(p_composition))
	{
		...
		for (const PlanClaim &claim : _plan.townClaims)
		{
			...
			for (int z = minZ; z <= maxZ; ++z)
			{
				for (int x = minX; x <= maxX; ++x)
				{
					...
					if (surface >= claim.min.y && surface <= claim.max.y)
					{
						_layers[_index(column)] |= townLayer(TownWorkspaceLayer::TerrainBlocked);
					}
				}
			}
		}
	...
	std::uint32_t TownWorkspace::layers(TownColumn p_column) const
	{
		return _layers[_index(p_column)];
	}
	bool TownWorkspace::has(TownColumn p_column, TownWorkspaceLayer p_layer) const
	{
		return (layers(p_column) & townLayer(p_layer)) != 0;
	}
	void TownWorkspace::add(TownColumn p_column, TownWorkspaceLayer p_layer)
	{
		_layers[_index(p_column)] |= townLayer(p_layer);
	}
	void TownWorkspace::remove(TownColumn p_column, TownWorkspaceLayer p_layer)
	{
		_layers[_index(p_column)] &= ~townLayer(p_layer);
	}
	int TownWorkspace::surfaceHeight(TownColumn p_column) const
	{
		return _surfaceHeights[_index(p_column)];
	}
