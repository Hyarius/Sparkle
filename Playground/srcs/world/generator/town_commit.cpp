...
namespace pg
{
	...
	{
		if (p_candidate.compositionId.empty()) throw std::invalid_argument("cannot commit a town without a composition id");
		for (const PlanTownRecord &town : p_plan.towns) if (town.macroEntityIndex == p_candidate.macroEntityIndex) throw std::logic_error("settlement already owns a committed town");
		...
		for (const ResolvedTownEntrance &entrance : p_candidate.entrances)
		{
			...
			for (TownColumn approach : entrance.approachColumns) resolved.approachColumns.emplace_back(p_candidate.bounds.minX + approach.x, p_candidate.bounds.minZ + approach.z);
		...
		for (std::size_t i = 0; i < p_candidate.roadScenery.size(); ++i) record.roadSceneryPlacementIndices.push_back(firstRoadScenery + i);
		...
		for (std::size_t i = 0; i < p_candidate.groundScenery.size(); ++i) record.groundSceneryPlacementIndices.push_back(firstGroundScenery + i);
		...
		for (const PlanPavedColumn &path : p_candidate.urbanRoadSurface)
		{
			...
			if (!merged.townPath.contains(row, col)) throw std::out_of_range("urban road lies outside the world plan");
