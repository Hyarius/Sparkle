...
namespace pg
{
	CameraControllerLogic::CameraControllerLogic(GameContext &p_context, Camera3D &p_camera) :
		_context(p_context), _camera(p_camera)
	{
	}
	...
	void CameraControllerLogic::_parseComponentForUpdate(const spk::UpdateTick &p_tick, Actor &p_actor)
	{
		...
		{
			if ((*p_tick.keyboard)[spk::Keyboard::Q] == spk::InputState::Down) _yaw -= 75.0f * seconds;
			if ((*p_tick.keyboard)[spk::Keyboard::D] == spk::InputState::Down) _yaw += 75.0f * seconds;
			if ((*p_tick.keyboard)[spk::Keyboard::Z] == spk::InputState::Down) _pitch += 55.0f * seconds;
			if ((*p_tick.keyboard)[spk::Keyboard::S] == spk::InputState::Down) _pitch -= 55.0f * seconds;
		}
		_pitch = std::clamp(_pitch, 20.0f, 75.0f);

		const Transform3D *transform = p_actor.entity() == nullptr ? nullptr : p_actor.entity()->component<Transform3D>();
		...
		_camera.setPosition(_smoothedTarget + offset);
	}
	...
	void CameraControllerLogic::_parseComponentForMouseMovedEvent(spk::MouseMovedEvent &p_event, Actor &p_actor)
	{
		if (!_context.world.explorationActive || !p_actor.player ||
			p_event.device()[spk::Mouse::Right] != spk::InputState::Down) return;
