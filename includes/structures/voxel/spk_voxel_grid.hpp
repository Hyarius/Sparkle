...
namespace spk
{
	...
	private:
		...
		std::vector<spk::VoxelCell> _cells;
	...
	public:
		...
		explicit VoxelGrid(const spk::Vector3Int &p_size, const spk::VoxelCell &p_fill = {}) :
			_size(p_size),
		...
		[[nodiscard]] spk::VoxelCell &cell(const spk::Vector3Int &p_position)
		{
		...
		[[nodiscard]] spk::VoxelCell &cell(int p_x, int p_y, int p_z)
		{
		...
		[[nodiscard]] const std::vector<spk::VoxelCell> &cells() const noexcept
		{
			return _cells;
		}
	};
