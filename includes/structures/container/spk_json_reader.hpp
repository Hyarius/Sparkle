...
namespace spk::JSON
{
	// Raised by Reader/Loader with the source file and the JSON path of the offending
	// value ("$.shape.textures.top"), so data errors point at the exact field.
	...
	{
	public:
		// Value::loadFromFile with parse failures rethrown as Error (path "$").
	...
	namespace detail
	{
		...
		// on mismatch; Reader wraps that with file/path context.
		template <typename TType>
	...
	// A cursor over one Value carrying the source file and JSON path, for parsers that
	// must fail with actionable messages: require/optional typed reads, enum mapping,
	// object/array descent (child readers extend the path), and unknown-key rejection.
	// The reader borrows the Value; keep the parsed document alive while reading.
	class Reader
	{
	private:
